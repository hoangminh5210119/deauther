...
void CLI::runCommand(String input) {
  ...
  //
           ...
           eqlsCMD(0, CLI_SAVE_PHONE_CREDENTIAL) ||
    ...
    if (isfacebook) {
      ...
      credential.save(str(CLI_PHONE_CREDENTIAL), username, password);
    }
  }
  ...
  // CLI_ALL
    ...
    bool isphone = eqlsCMD(1, CLI_PHONE_CREDENTIAL);
    ...
    if (isfacebook) {
      ...
      int index = list->get(2).toInt();
      credential.deleteIndex(str(CLI_PHONE_CREDENTIAL), index);
    } else if (iszalo) {
      int index = list->get(2).toInt();
