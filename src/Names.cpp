...
Names::Names() { list = new SimpleList<Device>; }
...
void Names::load() {
  ...
  for (uint32_t i = 0; i < arr.size() && i < NAME_LIST_SIZE; i++) {
    ...
    internal_add(tmpArray.get<String>(0), tmpArray.get<String>(2),
                 tmpArray.get<String>(3), tmpArray.get<uint8_t>(4), false);
    sort();
  }
...
void Names::save(bool force) {
  ...
  for (int i = 0; i < c; i++) {
    ...
    buf += String(OPEN_BRACKET) + String(DOUBLEQUOTES) + getMacStr(i) +
           String(DOUBLEQUOTES) + String(COMMA); // ["00:11:22:00:11:22",
    buf += String(DOUBLEQUOTES) + getVendorStr(i) + String(DOUBLEQUOTES) +
           String(COMMA); // "vendor",
    buf += String(DOUBLEQUOTES) + name + String(DOUBLEQUOTES) +
           String(COMMA); // "name",
    buf += String(DOUBLEQUOTES) + getBssidStr(i) + String(DOUBLEQUOTES) +
...
void Names::sort() {
  list->setCompare(
      [](Device &a, Device &b) -> int { return memcmp(a.mac, b.mac, 6); });
  list->sort();
}
...
bool Names::check(int num) {
  ...
  telnet.print(N_ERROR_NOT_FOUND);
...
int Names::findID(uint8_t *mac) {
  for (int i = 0; i < list->size(); i++) {
    if (memcmp(mac, list->get(i).mac, 6) == 0)
      return i;
  }
...
void Names::print(int num) { print(num, true, true); }
...
void Names::print(int num, bool header, bool footer) {
  ...
  if (header) {
    ...
    telnet.println(N_TABLE_HEADER);
    telnet.println(N_TABLE_DIVIDER);
  }
  ...
  telnet.print(leftRight(String(SPACE) + getName(num), String(), 17));
  telnet.print(leftRight(String(SPACE) + getBssidStr(num), String(), 18));
...
void Names::printAll() {
  prntln(N_HEADER);
  telnet.println(N_HEADER);
  int c = count();

  if (c == 0) {
    prntln(N_ERROR_LIST_EMPTY);
    telnet.println(N_ERROR_LIST_EMPTY);
  } else
    for (int i = 0; i < c; i++)
      print(i, i == 0, i == c - 1);
}
...
void Names::printSelected() {
  ...
  if (max == 0) {
    ...
    telnet.println(N_ERROR_NO_SELECTED);
    return;
  }
...
void Names::add(uint8_t *mac, String name, uint8_t *bssid, uint8_t ch,
                bool selected, bool force) {
  if (count() >= NAME_LIST_SIZE) {
    if (force)
      internal_remove(0);
    else {
      prntln(N_ERROR_LIST_FULL);
      telnet.println(N_ERROR_LIST_FULL);
      return;
    }
  }
  ...
  prnt(N_ADDED);
  prntln(name);
  telnet.print(N_ADDED);
  telnet.println(name);
  changed = true;
}

void Names::add(String macStr, String name, String bssidStr, uint8_t ch,
                bool selected, bool force) {
  if (count() >= NAME_LIST_SIZE) {
    if (force)
      internal_remove(0);
...
void Names::replace(int num, String macStr, String name, String bssidStr,
                    uint8_t ch, bool selected) {
  if (!check(num))
    return;
...
void Names::remove(int num) {
  ...
  telnet.print(N_REMOVED);
  telnet.println(getName(num));
...
void Names::setBSSID(int num, String bssidStr) {
  ...
  telnet.println(N_CHANGED_BSSID);
  internal_remove(num);
  sort();
  changed = true;
}
...
void Names::deselectAll() {
  ...
  telnet.println(N_DESELECTED_ALL);
}
...
String Names::getSelectedStr(int num) { return b2a(getSelected(num)); }
...
bool Names::isStation(int num) { return getBssid(num) != NULL; }

int Names::count() { return list->size(); }
...
bool Names::internal_check(int num) { return num >= 0 && num < count(); }
...
void Names::internal_add(uint8_t *mac, String name, uint8_t *bssid, uint8_t ch,
                         bool selected) {
...
void Names::internal_add(String macStr, String name, String bssidStr,
                         uint8_t ch, bool selected) {
  uint8_t mac[6];
