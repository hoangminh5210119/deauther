...
void SSIDs::print(int num, bool header, bool footer) {
  ...
  if (header) {
    ...
    telnet.println(SS_TABLE_HEADER);
    telnet.println(SS_TABLE_DIVIDER);
  }
  ...
  telnet.println(leftRight(String(SPACE) + getName(num), String(), 33));
...
void SSIDs::printAll() {
  prntln(SS_HEADER);
  telnet.println(SS_HEADER);
  int c = count();

  if (c == 0) {
    prntln(SS_ERROR_EMPTY);
    telnet.println(SS_ERROR_EMPTY);
  } else
    for (int i = 0; i < c; i++)
      print(i, i == 0, i == c - 1);
}
...
void SSIDs::enableRandom(uint32_t randomInterval) {
  ...
  telnet.println(SS_RANDOM_ENABLED);
  update();
}
...
void SSIDs::disableRandom() {
  ...
  telnet.println(SS_RANDOM_DISABLED);
}
