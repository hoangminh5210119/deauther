...
void Scan::start(uint8_t mode, uint32_t time, uint8_t nextmode,
  ...
  /* Station Scan */
    ...
    telnet.println(SC_START_CLIENT);

    if (sniffTime > 0) {
      prnt(String(sniffTime / 1000) + S);
      telnet.print(String(sniffTime / 1000) + S);
    } else {
      prnt(SC_INFINITELY);
      telnet.print(SC_INFINITELY);
    }

    if (!channelHop) {
      prnt(SC_ON_CHANNEL);
      prnt(wifi_channel);
      telnet.print(SC_ON_CHANNEL);
      telnet.print(String(wifi_channel));
    }
    prntln();
    ...
    telnet.print(SC_ON_CHANNEL);
    telnet.println(channelHop ? str(SC_ONE_TO) + (String)14 : (String)wifi_channel);
  ...
  /* Stop scan */
    ...
    if (scan_continue_mode != SCAN_MODE_OFF) {
      ...
      prntln(SC_CONTINUE);
      telnet.print(SC_RESTART);
      telnet.print(String(int(continueTime / 1000)));
      telnet.println(SC_CONTINUE);
    }
  }
  ...
  /* ERROR */
    ...
    prnt(SC_ERROR_MODE);
    prntln(mode);
    telnet.print(SC_ERROR_MODE);
    telnet.println(String(mode));
    return;
  }
}
...
void Scan::update() {
  ...
  if (isSniffing()) {
    ...
    // channel hopping
    if (channelHop &&
        (currentTime - snifferChannelTime > settings.getChTime())) {
