...
void Settings::load() {
  ...
  if (data.containsKey(keyword(S_LEDENABLED)))
    setLedEnabled(data.get<bool>(keyword(S_LEDENABLED)));
...
void Settings::set(const char *str, String value) {
  ...
  else if (eqls(str, S_LEDENABLED))
    setLedEnabled(s2b(value));
  ...
  else if (eqls(str, S_VERSION))
    prntln(S_ERROR_VERSION);
...
// ===== GETTERS ===== //
String Settings::getVersion() { return version; }
...
bool Settings::getLedEnabled() { return ledEnabled; }
